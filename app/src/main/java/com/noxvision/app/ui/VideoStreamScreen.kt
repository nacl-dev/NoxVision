                ...
                Box(
                    ...
                    if (isPlaying) {
                        ...
                        if (objectDetectionEnabled && detectedObjects.isNotEmpty()) {
                            // ⚡ Bolt: Hoisted Paint and Rect outside of Canvas using remember.
                            // Impact: Eliminates per-frame memory allocation for text rendering,
                            // significantly reducing GC churn and frame drops during live video stream.
...
private fun CrosshairOverlay(style: CrosshairStyle) {
    // ⚡ Bolt: Hoisted Path outside of Canvas using remember.
    // Impact: Prevents allocating a new Path on every frame draw,
    // reducing GC overhead for dynamic crosshairs.
